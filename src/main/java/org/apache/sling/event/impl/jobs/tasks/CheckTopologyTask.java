                        ...
                        while (caps.isActive() && topicIter.hasNext()) {
                            ...
                            if (reassign) {
                                ...
                                JobTopicTraverser.traverse(
                                        this.logger, topicResource, new JobTopicTraverser.ResourceCallback() {
                                                ...
                                                try {
                                                    ...
                                                    try {
                                                        ...
                                                        if (targetId != null) {
                                                            configuration
                                                                    .getAuditLogger()
                                                                    .debug("REASSIGN OK {} : {}", targetId, jobId);
                                                        } else {
                                                            configuration
                                                                    .getAuditLogger()
                                                                    .debug("REUNASSIGN OK : {}", jobId);
                                                        }
                                                    } catch (final PersistenceException pe) {
                                                        logger.warn(
                                                                "Unable to move stale job from " + rsrc.getPath()
                                                                        + " to " + newPath,
                                                                pe);
                                                        resolver.refresh();
                                                        resolver.revert();
                ...
                JobTopicTraverser.traverse(this.logger, topicResource, new JobTopicTraverser.ResourceCallback() {
                    ...
                    public boolean handle(final Resource rsrc) {
                        try {
                            ...
                            if (targetId != null) {
                                final String newPath = configuration.getAssginedJobsPath()
                                        + '/'
                                        + targetId
                                        + '/'
                                        + topicResource.getName()
                                ...
                                try {
                                    ...
                                    logger.warn(
                                            "Unable to move unassigned job from " + rsrc.getPath() + " to " + newPath,
                                            pe);
                                    resolver.refresh();
                                    resolver.revert();
                                }
            ...
            if (caps.isActive() && unassign) {
                ...
                JobTopicTraverser.traverse(this.logger, topicResource, new JobTopicTraverser.ResourceCallback() {
                    ...
                    public boolean handle(final Resource rsrc) {
                        try {
                            final ValueMap vm = ResourceHelper.getValueMap(rsrc);
                            final String newPath = configuration.getUnassignedJobsPath()
                                    + '/'
                                    + topicResource.getName()
                                    + rsrc.getPath()
