...
public class JobHandler {
    ...
    public JobHandler(final JobImpl job, final JobExecutor consumer, final JobManagerConfiguration configuration) {
        this.job = job;
    ...
    public boolean reschedule() {
        return withJobResource((jobResource, mvm) -> {
            ...
            try {
                ...
                this.configuration
                        .getMainLogger()
                        .debug("Unable to update reschedule properties for job " + job.getId(), pe);
            }
            return false;
        });
    }
            ...
            try {
                ...
                if (keepJobInHistory && configuration.getMainLogger().isDebugEnabled()) {
                    if (isSuccess) {
                        configuration
                                .getMainLogger()
                                .debug("Kept successful job {} at {}", Utility.toString(job), newPath);
                    } else {
                        configuration
                                .getMainLogger()
                                .debug("Moved cancelled job {} to {}", Utility.toString(job), newPath);
                    }
                }
            } catch (final PersistenceException pe) {
    ...
    public void reassign() {
        ...
        final String targetId =
                (caps == null ? null : caps.detectTarget(job.getTopic(), job.getProperties(), queueInfo));

        withJobResource((jobResource, mvm) -> {
            final String newPath =
                    this.configuration.getUniquePath(targetId, job.getTopic(), job.getId(), job.getProperties());
        ...
        try (ResourceResolver resolver = this.configuration.createResourceResolver()) {
            ...
            if (mvm == null) {
                this.configuration
                        .getMainLogger()
                        .debug(
                                "Cannot adapt resource {} to ModifiableValueMap, no write permissions?",
                                job.getResourcePath());
                return false;
            }
