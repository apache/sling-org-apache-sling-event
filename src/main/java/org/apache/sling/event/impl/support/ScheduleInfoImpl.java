...
public class ScheduleInfoImpl implements ScheduleInfo, Serializable {
    ...
    public static ScheduleInfoImpl deserialize(final ScheduleType scheduleType, final String s) {
        ...
        if (scheduleType == ScheduleType.YEARLY && parts.length == 4) {
            ...
            try {
                return new ScheduleInfoImpl(
                        scheduleType, -1, Integer.parseInt(parts[0]), Integer.parseInt(parts[1]), null, -1, null);
            } catch (final IllegalArgumentException iae) {
                // ignore and return null
            }
    ...
    public int getDayOfMonth() {
        return (this.scheduleType == ScheduleType.MONTHLY || this.scheduleType == ScheduleType.YEARLY
                ? this.dayOfWeek
                : -1);
    }
    ...
    public void check(final List<String> errors) {
        switch (this.scheduleType) {
            case DAILY:
                if (hourOfDay < 0 || hourOfDay > 23 || minuteOfHour < 0 || minuteOfHour > 59) {
                    errors.add("Wrong time information : " + minuteOfHour + ":" + minuteOfHour);
                }
                break;
            case DATE:
                if (at == null || at.getTime() <= System.currentTimeMillis() + 2000) {
